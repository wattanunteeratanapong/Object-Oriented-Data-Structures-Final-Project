...
# Function to flatten the input list
...
# Function to open the add guest window (popup)
...
# Function to show the guest list in a new popup
def show_guest_list():
    ...
    # Create a label to display the guest list
    label = tk.Label(guest_popup, text="Current Guest List:", font=("Arial", 14))
    label.pack(pady=10)

    # Display the guest list in a Text widget
    text_box = tk.Text(guest_popup, height=20, width=50)
    text_box.pack(pady=10)

    # Insert the guest list into the text box
    for guest in guest_list:
        text_box.insert(tk.END, guest + "\n")

    text_box.config(state=tk.DISABLED)  # Make the text box read-only

# Function to delete a guest from a room
...
# Function to search for a guest in a room
...
# Function to show empty rooms
def show_empty_rooms_popup():
    popup = tk.Toplevel(root)
    popup.title("Empty Rooms")
    popup.geometry("400x400+220+130")
    ...
    text_box = tk.Text(popup, height=20, width=50)
    text_box.pack(pady=10)
